...
var vite_config_default = defineConfig(({ mode }) => {
  const viteConfig = mode === "dev" ? DEV_CONFIG : mode === "docs" ? DOCS_CONFIG : PACKAGES_CONFIG;
  return {
    ...viteConfig.config,
    resolve: {
      alias: {
        "@": fileURLToPath(new URL("./packages", __vite_injected_original_import_meta_url)),
        "@docs": fileURLToPath(new URL("./docs", __vite_injected_original_import_meta_url))
      }
    },
    plugins: [
      ...viteConfig.plugins,
